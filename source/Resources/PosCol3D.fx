...
float4 PSPoint(VS_OUTPUT input) : SV_TARGET
{
	...
	const float3 normalColor = gNormalMap.Sample(samPoint, input.UV).xyz;
	...
	// phong specular
	const float3 gloss = gGlossinessMap.Sample(samPoint, input.UV).xyz;
	const float exponent = gloss.x * gShininess;
	float3 specular = Phong(gSpecularMap.Sample(samPoint, input.UV).xyz, exponent, -gLightDirection, viewDirection, normal);
	////////////////////////

	float3 diffuse = Lambert(gDiffuseMap.Sample(samPoint, input.UV).xyz);
...
float4 PSLinear(VS_OUTPUT input) : SV_TARGET
{
	...
	const float3 normalColor = gNormalMap.Sample(samLinear, input.UV).xyz;
	float3 sampledNormal = 2.f * normalColor - float3( 1, 1, 1 ); // => [0, 1] to [-1, 1]
	...
	// phong specular
	const float3 gloss = gGlossinessMap.Sample(samLinear, input.UV).xyz;
	const float exponent = gloss.x * gShininess;
	float3 specular = Phong(gSpecularMap.Sample(samLinear, input.UV).xyz, exponent, -gLightDirection, viewDirection, normal);
	////////////////////////

	float3 diffuse = Lambert(gDiffuseMap.Sample(samLinear, input.UV).xyz);
...
float4 PSAnisotropic(VS_OUTPUT input) : SV_TARGET
{
	...
	const float3 normalColor = gNormalMap.Sample(samAnisotropic, input.UV).xyz;
	float3 sampledNormal = 2.f * normalColor - float3( 1, 1, 1 ); // => [0, 1] to [-1, 1]
	...
	// phong specular
	const float3 gloss = gGlossinessMap.Sample(samAnisotropic, input.UV).xyz;
	const float exponent = gloss.x * gShininess;
	float3 specular = Phong(gSpecularMap.Sample(samAnisotropic, input.UV).xyz, exponent, -gLightDirection, viewDirection, normal);
	////////////////////////

	float3 diffuse = Lambert(gDiffuseMap.Sample(samAnisotropic, input.UV).xyz);
